            ...
            <div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-5 md:gap-10 overflow-y-scroll no-scrollbar overflow-auto p-1 rounded-lg'>
                {
                    movies.map((movie, index) => (
                        <img key={index} src={`https://image.tmdb.org/t/p/w500${movie.poster_path}`} className='h-40 md:h-55 w-30 md:w-40 rounded-lg object-cover hover:scale-105 transition-transform duration-300' onClick={() => navigate(`movie/${movie?.id}`)}></img>
                    ))
                }
            </div>
