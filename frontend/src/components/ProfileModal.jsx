...
function ProfileModal({ setOpenProfileModal, openProfileModel }) {
    ...
    return (
        <div className='fixed'>
            <div className='bg-[#2e2e2e] rounded-lg p-5 flex flex-col gap-5 items-center'>
                <div className='flex justify-between w-full items-center'>
                    <p className='font-semibold text-2xl'>Choose a Picture</p>
                    <RotateCcw size={16} onClick={() => setRandom(Math.random())} />
                </div>
                <div className='grid grid-cols-5 gap-5'>
                    {
                        [10, 3, 32, 41, 57, 64, 79, 82, 95, 101].map((val, index) => (
                            <div className='flex flex-col justify-end items-end'>
                                <img src={`https://api.dicebear.com/9.x/glass/svg?seed=${val * random}`} className='size-18 rounded-full hover:scale-105 duration-300 ease-in-out' key={index} onClick={() => setSelected(index)} />
                                <div className={`bg-white p-1 rounded-full absolute justify-end ${selected === index ? `opacity-100` : `opacity-0`} shadow-md transition-opacity duration-500 ease-in-out`}>
