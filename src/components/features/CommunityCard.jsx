...
export default function CommunityCard({ topReviews = [] }) {
    ...
    <div className="grid md:grid-cols-2 gap-4 md:gap-5 items-stretch">
      {topReviews.map((review, index) => (
        <div
          ...
          className="bg-[#60606071] hover:bg-[#393939] ease-in-out duration-300 rounded-lg p-3 flex flex-col gap-3 cursor-pointer"
          onClick={() => router.push(`/movie/${review.tmdbId}`)}
        >
          ...
          <div className="flex gap-4 h-full">
            ...
            <div className="flex flex-col flex-1">
              <div className="flex flex-col gap-2">
                <p className="font-bold">{review.movieName}</p>
                ...
                <p className="text-sm leading-relaxed text-gray-200 line-clamp-6">{review.text}</p>
              </div>
