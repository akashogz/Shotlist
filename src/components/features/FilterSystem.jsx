...
export default function FilterSystem({ setFilters }) {
  const { localFilters, setLocalFilters, activeFilters, setActiveFilters, clearAll } = useUserStore();
  const [openDropdown, setOpenDropdown] = useState(null);

  useEffect(() => {
    setFilters({ ...localFilters, with_genres: localFilters.genres.join(",") });
  }, [localFilters, setFilters]);

  const removeFilter = useCallback(() => { clearAll(); }, [clearAll]);

  const handleSelect = (id, label) => {
    const isGenre = id === "genre";
    const exists = activeFilters.some((f) => f.id === id && f.label === label);
    if (exists) { removeFilter(); } else {
      ...
      setLocalFilters((prev) => {
        ...
        if (id === "genre") { const gid = GENRE_MAP[label]; if (!updated.genres.includes(gid)) updated.genres = [...updated.genres, gid]; }
        if (id === "year") {
          const yearMap = {
    ...
    <div className="w-full text-white">
          ...
          {FILTERS.map((f) => (
            <div key={f.id} className="relative">
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 bg-[#505050] rounded-full text-xs border transition-colors ${activeFilters.some((af) => af.id === f.id) ? "border-white" : "border-[#606060]"}`}
              >
                ...
                <div className="absolute top-12 left-0 bg-[#1A1A1A] border border-[#333] rounded-xl shadow-2xl z-50 p-1 min-w-40">
                  {f.options.map((opt) => (
                    <div
                      ...
                      className={`p-3 rounded-lg cursor-pointer text-sm ${activeFilters.some((af) => af.label === opt) ? "bg-white text-black" : "hover:bg-[#262626]"}`}
                    >
                      {opt}
                    </div>
                  ))}
      ...
      <aside className="hidden lg:flex flex-col w-72 bg-[#303030] border-r border-[#262626] h-screen fixed left-0 top-0 pt-24 p-6 overflow-y-auto no-scrollbar">
        <div className="flex justify-between items-center mb-8">
          ...
          {activeFilters.length > 0 && (
            <button onClick={clearAll} className="text-xs text-gray-400 hover:text-white uppercase tracking-tighter">Reset All</button>
          )}
        </div>
        {FILTERS.map((section) => (
          <div key={section.id} className="mb-8 border-b border-[#444] pb-6">
            <h3 className="text-[10px] font-bold text-gray-500 uppercase mb-4 tracking-widest">{section.title}</h3>
              ...
              {section.options.map((opt) => {
                ...
                return (
                  <button
                    ...
                    className={`text-[11px] px-3 py-1.5 rounded-full border transition-all ${isActive ? "bg-white text-black border-white font-bold" : "border-[#505050] text-gray-400 hover:border-white"}`}
                  >
                    {opt}
                  </button>
                );
