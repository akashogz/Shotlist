...
import GenreCards from "@/components/features/GenreCards";
...
export default function Home() {
  ...
  function prevSlide() {
    setCurrentBG((prev) => (slides.length ? (prev - 1 + slides.length) % slides.length : 0));
  }
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const [popularData, trendingData, allTimeData] = await Promise.all([
  ...
  return (
    <>
      ...
      >
        {heroLoading && <div className="absolute inset-0 animate-pulse bg-gradient-to-b from-[#55555584] to-[#24242476]" />}
        ...
        {!heroLoading && (
          <>
            {slides?.map((slide, index) => (
              <img
                key={slide.id}
                src={slide.backdrop_path ? `https://image.tmdb.org/t/p/w1280${slide.backdrop_path}` : "/fallback.jpg"}
                alt={slide.title || "Movie backdrop"}
                loading="eager"
                className={`absolute inset-0 w-full h-full object-cover object-center transition-opacity duration-700 ease-in-out ${index === currentBG ? "opacity-100" : "opacity-0 pointer-events-none"}`}
              />
            ))}
            <div className="absolute inset-0 bg-gradient-to-b from-[#464e8263] to-[#242424]" />
            <div className="relative z-10 h-full flex flex-col justify-end px-5 md:px-20 gap-6 text-white bottom-20 md:bottom-25">
              <h1 className="text-4xl md:text-6xl font-black drop-shadow-md">{slides[currentBG]?.title}</h1>
              <p className="text-md md:text-lg drop-shadow-md line-clamp-4">{slides[currentBG]?.overview}</p>
              <div className="flex gap-2">
                <Link href={`/movie/${slides[currentBG]?.id}?trailer=open`} className="border rounded-full p-3 px-4 flex items-center gap-2 text-[14px] font-bold cursor-pointer">
                  Play Trailer <Play size={14} fill="white" />
                </Link>
                <Link href={`/movie/${slides[currentBG]?.id}`} className="border rounded-full p-3 px-4 flex items-center gap-2 text-[14px] bg-[#464E82] border-[#464E82] font-bold cursor-pointer">
              ...
              <div className="w-full flex items-center justify-center gap-4 px-20">
                {slides?.map((_, index) => (
                  <button
                    ...
                    className={`w-2 h-2 rounded-full -mb-10 transition-opacity duration-700 ${index === currentBG ? "bg-white opacity-100" : "bg-white/50 opacity-50"}`}
                  />
                ))}
              </div>
            </div>
      ...
      <div className="px-5 md:px-20 flex flex-col gap-8 mt-5">
        ...
        <section>
          ...
          <CommunityCard topReviews={topReviews} />
        </section>
      </div>
      <Footer />
    </>
